...
class AcquisitionUtility():
    ...
    def minimise_acq_function(self, hyper_gaussian_object, iteration_count):

        self.hyper_gaussian_object = hyper_gaussian_object
        # Initialize the xmax value and the function values to zeroes

        min_basis_weights = np.zeros(self.hyper_gaussian_object.number_of_basis_vectors_chosen)
        ...
        if self.acq_type == "UCB":
            ...
            for starting_point in random_starting_points:
                ...
                min_weights = opt.minimize(lambda x: self.upper_confidence_bound_util(x, self.hyper_gaussian_object,  iteration_count),
                ...
                mean, variance = self.hyper_gaussian_object.predict_for_hypergp(np.array([min_weights['x']]))
                std_dev = np.sqrt(variance)
                ...
                tempweights_lambda.append(min_weights['x'])
                ...
                # Commenting to calculate the minimum negative likelihood
                # if fvalue > function_best:
                if fvalue < function_best:
                    PH.printme(PH.p1, "New best Fval: ", fvalue, " found at: ", min_weights['x'])
                    min_basis_weights = min_weights['x']
                    function_best = fvalue

            PH.printme(PH.p1, "UCB Best is ", function_best, "at ", min_basis_weights)
        return min_basis_weights
